...
def build_universe(api, cfg):
    info = pd.DataFrame(api.listed_info())
    info["code"] = info["Code"].astype(str).str[:4]
    u = cfg["universe"]
    info = info[info["MarketCode"].astype(str).isin(u["market_codes"])]
    info = info[~info["Sector33Code"].astype(str).isin(u["exclude_sector33"])]
    info = info.drop_duplicates(subset=["code"], keep="last").set_index("code")
    return pd.DataFrame({
        "name": info["CompanyName"],
        "sector": info.get("Sector33CodeName", pd.Series(dtype=str)),
        "market": info.get("MarketCodeName", pd.Series(dtype=str)),
    })


def is_trading_day_jst(api, today):
    try:
        cal = api.trading_calendar(today.isoformat(), today.isoformat())
        if cal:
            return str(cal[0].get("HolidayDivision")) in ("1", "2")
    except Exception:
        pass
...
def run(mode):
    ...
    if mode == "intraday" and not is_trading_day_jst(api, today):
        print("[run] 本日は非営業日のため日中更新は行いません")
        return

    prices = fetch.update_prices(api, today)
    stmts = fetch.update_statements(api, today)
    ...
    if mode == "intraday":
        # 日中は「もう少しで条件を満たす」銘柄まで含めて株価を取り直す
        req = cfg["required"]
        ...
        watch = watch.sort_values("avg_turnover", ascending=False).head(500)
