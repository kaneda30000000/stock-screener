...
@media(min-width:760px){
  ...
  .tabs,h1,.sub,.stats,.hint,.alert,details.about,footer{flex:none}
...
.tab{
  ...
  font-size:14px;font-weight:600;color:var(--ink3);text-decoration:none;
  padding:8px 14px 9px;margin-bottom:-1px;white-space:nowrap;
}
a.tab:hover{color:var(--ink2);background:var(--line2);border-radius:8px 8px 0 0}
.tab.on{color:var(--accent);border-bottom-color:var(--accent)}
.tab .ext{font-size:11px;opacity:.6;margin-left:3px}
@media(max-width:560px){.tab{padding:8px 10px 9px;font-size:13px}}
"""
...
def render_page(passed, allrows, cfg, meta, out_path):
    ...
    burl = str(cfg["display"].get("brief_url") or "")
    brief_tab = (f'<a class="tab" href="{html.escape(burl, quote=True)}">'
                 '朝のブリーフ<span class="ext">↗</span></a>') if burl else ""
...
<div class="wrap">
  <nav class="tabs">
    <span class="tab on">スクリーニング</span>
    {brief_tab}
  </nav>
  <h1>買い時スクリーニング</h1>
